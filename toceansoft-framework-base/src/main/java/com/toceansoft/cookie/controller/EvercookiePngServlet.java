 ...
 * 
 * Install this servlet at /evercookie_png.php in your web.xml (or add
 ...
 * 
 */
// @WebServlet(name = "EvercookiePngServlet", urlPatterns = {
// "/api/cookie/php/evercookie_png.php" }, loadOnStartup = 1)
public class EvercookiePngServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		image.createGraphics().setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_OFF);
		int x = 0;

		for (int i = 0; i < cookieValue.length(); i += 3) {
			// Treat every 3 chars of the cookie value as an {R,G,B} triplet.
			Color c = new Color(cookieValue.charAt(i), cookieValue.charAt(i + 1),
					cookieValue.charAt(i + 2));
