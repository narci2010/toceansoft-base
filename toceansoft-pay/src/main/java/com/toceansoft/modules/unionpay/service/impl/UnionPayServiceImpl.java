 ...
 * 修改内容：  
 */
package com.toceansoft.modules.unionpay.service.impl;
...
import com.toceansoft.modules.unionpay.util.SDKConfig;
import com.toceansoft.modules.unionpay.util.UnionConfig;
...
public class UnionPayServiceImpl implements IUnionPayService {
	...
	public String unionPay(Product product) {
		...
		requestData.put("encoding", UnionConfig.ENC_UTF8); // 字符集编码，可以使用UTF-8,GBK两种方式
		requestData.put("signMethod", "01"); // 签名方法，只支持 01：RSA方式证书加密
		requestData.put("txnType", "01"); // 交易类型 ，01：消费
		requestData.put("txnSubType", "01"); // 交易子类型， 01：自助消费
		requestData.put("bizType", "000201"); // 业务类型，B2C网关支付，手机wap支付
		// 渠道类型，这个字段区分B2C网关支付和手机wap支付；07：PC,平板 08：手机
		if (product.getPayWay().equals(PayWay.MOBILE.getCode())) { // 手机支付
		...
		/*** 商户接入参数 测试账号 ***/
		requestData.put("merId", UnionConfig.MER_ID); // 商户号码，请改成自己申请的正式商户号或者open上注册得来的777测试商户号
		...
		Map<String, String> submitFromData = AcpService.sign(requestData, UnionConfig.ENC_UTF8);
		// 报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。

		String requestFrontUrl = SDKConfig.getConfig().getFrontRequestUrl(); // 获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acpsdk.frontTransUrl
		String form = AcpService.createAutoFormHtml(requestFrontUrl, submitFromData,
				UnionConfig.ENC_UTF8); // 生成自动跳转的Html表单
	...
	public void fileTransfer() {
		...
		data.put("encoding", UnionConfig.ENC_UTF8);
		// 签名方法
		...
		data.put("merId", UnionConfig.MER_ID);
		...
		Map<String, String> reqData = AcpService.sign(data, UnionConfig.ENC_UTF8);
		// 报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。
		...
		Map<String, String> rspData = AcpService.post(reqData, url, UnionConfig.ENC_UTF8);

		if (!rspData.isEmpty()) {

			if (AcpService.validate(rspData, UnionConfig.ENC_UTF8)) {
