 ...
 * 修改内容：  
 */
package com.toceansoft.modules.alipay.service.impl;
...
/**
 * 支付宝 创建者 拓胜科技 创建时间 2018年1月15日 ====================== 商户端私钥：
...
public class AliPayServiceImpl implements IAliPayService {
	...
	public String aliPay(Product product) {
		...
		// 二维码存放路径
		log.debug(Constants.QRCODE_PATH);
		...
		AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder().setSubject(subject)
				.setTotalAmount(totalAmount).setOutTradeNo(outTradeNo).setSellerId(sellerId).setBody(body)
				// 128长度
				// --附加信息
		...
		try {
			...
			if (file.exists()) {
				...
				if (response.isSuccess()) { // 扫码未支付的情况
					...
					// delete()方法返回false，如果文件无法删除成功（而不是抛出一个例外）。
