 ...
 * 文件名：DefaultHeaderSessionManager.java
 * 描述：  
 * 修改人： Narci.Lee  
 * 修改时间：2017年11月22日  
 * 跟踪单号：  
 ...
 */
@Slf4j
public class DefaultHeaderSessionManager extends DefaultSessionManager
	...
	private String getSessionIdHeaderValue(ServletRequest request, ServletResponse response) {
		if (!(request instanceof HttpServletRequest)) {
			...
			if (StringUtils.isNotEmpty(sessionId)) {
				try {
					sessionId = URLEncoder.encode(sessionId, StandardCharsets.UTF_8.displayName());
	 ...
	 * response header中
	 */
	private void storeSessionId(Serializable currentId, HttpServletRequest ignored,
		...
		if (!WebUtils.isHttp(context)) {
			log.debug(
					"SessionContext argument is not HTTP compatible or does not have an HTTP request/response pair."
							+ " No session ID cookie will be set.");
		} else {
	...
	private void onInvalidation(SessionKey key) {
		...
		if (WebUtils.isHttp(key)) {
			...
			log.debug(
					"SessionKey argument is not HTTP compatible or does not have an HTTP request/response pair."
	...
	protected void onStop(Session session, SessionKey key) {
		...
		if (WebUtils.isHttp(key)) {
			...
			log.debug(
					"SessionKey argument is not HTTP compatible or does not have an HTTP request/response pair."
							+ " Session ID cookie will not be removed due to stopped session.");
		}
	}
