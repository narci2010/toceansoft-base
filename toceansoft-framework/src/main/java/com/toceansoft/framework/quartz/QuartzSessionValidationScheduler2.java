 ...
 * 文件名：QuartzSessionValidationScheduler2.java
 * 描述：  
 * 修改人： Narci.Lee  
 * 修改时间：2017年11月22日  
 * 跟踪单号：  
		...
		try {
			...
			if (scheduler == null) {
				if (log.isWarnEnabled()) {
					//@formatter:off
					log.warn(
							"getScheduler() method returned a null Quartz scheduler, which is unexpected.  "
							+ "Please check your configuration and/or implementation.  Returning quietly since there "
							+ "is no validation job to remove (scheduler does not exist).");
					//@formatter:on
				}
