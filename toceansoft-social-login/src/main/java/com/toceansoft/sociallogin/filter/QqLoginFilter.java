 ...
 */
@Slf4j
public class QqLoginFilter extends AuthenticatingFilter {
	...
	protected AuthenticationToken createToken(ServletRequest request, ServletResponse response)
		...
		QqToken qqToken = new QqToken(openid, state);
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response,
		...
		if (ShiroUtils.isLogin() && (loginUrl.contains(Constant.QQ_LOGIN_URL))) {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response)
			throws Exception {
		log.debug("QqLoginFilter onAccessDenied:before isLogin.");
		if (ShiroUtils.isLogin()) {
			// 如果已经登陆成功，直接放行。
			return true;
		}
		log.debug("QqLoginFilter onAccessDenied:after isLogin:false");
