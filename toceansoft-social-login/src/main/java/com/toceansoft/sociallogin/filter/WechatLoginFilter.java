 ...
 */
@Slf4j
public class WechatLoginFilter extends AuthenticatingFilter {
	...
	protected AuthenticationToken createToken(ServletRequest request, ServletResponse response)
		...
		WechatToken wechatToken = new WechatToken(openid, state);
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response,
		...
		if (ShiroUtils.isLogin() && (loginUrl.contains(Constant.WECHAT_LOGIN_URL))) {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response)
			throws Exception {
		log.debug("WechatLoginFilter onAccessDenied:before isLogin.");
		if (ShiroUtils.isLogin()) {
			// 如果已经登陆成功，直接放行。
			return true;
		}
		log.debug("WechatLoginFilter onAccessDenied:after isLogin:false");
