...
import com.toceansoft.sociallogin.oauth.OauthOsc;
 ...
 */
// @Controller
@RequestMapping("/api/osc")
@Slf4j
public class OauthOscController {
	// OAuth2.0标准协议建议，利用state参数来防止CSRF攻击。可存储于session或其他cache中
	@Autowired
	private OauthOsc oauthOsc;
	...
	public String callback(HttpServletRequest request) {
		...
		// 获取用户信息
		try {
			JSONObject userInfo = oauthOsc.getUserInfoByCode(code);
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		// state就是一个随机数，保证安全
		try {
			String url = oauthOsc.getAuthorizeUrl(state);
			return "redirect:" + url;
