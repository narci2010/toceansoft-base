...
import com.toceansoft.sociallogin.oauth.OauthRenren;
 ...
 */
// @Controller
@RequestMapping("/api/renren")
@Slf4j
public class OauthRenrenController {
	// OAuth2.0标准协议建议，利用state参数来防止CSRF攻击。可存储于session或其他cache中
	@Autowired
	private OauthRenren oauthRenren;
	...
	public String callback(HttpServletRequest request) {
		...
		// 获取用户信息
		try {
			JSONObject userInfo = oauthRenren.getUserInfoByCode(code);
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		// state就是一个随机数，保证安全
		try {
			String url = oauthRenren.getAuthorizeUrl(state, "PC");
			return "redirect:" + url;
