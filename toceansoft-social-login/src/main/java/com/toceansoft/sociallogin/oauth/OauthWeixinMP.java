...
@Getter
@Slf4j
public class OauthWeixinMP extends Oauth {
	...
	@Value("${spring.social.wechatmp.app-id}")
	private String clientId;
	@Value("${spring.social.wechatmp.app-secret}")
	private String clientSecret;
	@Value("${spring.social.wechatmp.redirect-url}")
	private String redirectUri;
	// snsapi_userinfo:需要用户授权，能获取更多信息
	// snsapi_base:静默，但获取信息比较少
	@Value("${spring.social.wechatmp.scope}")
	 ...
	 * @throws IOException              ioe
	 */
	public OAuth2Token getBaseTokenByCode()
			throws IOException, KeyManagementException, NoSuchAlgorithmException, NoSuchProviderException {
		Map<String, String> params = new HashMap<String, String>();
		params.put("appid", getClientId());
		params.put("secret", getClientSecret());
		...
		// GET POST都行
		String token = super.doPost(BASE_TOKEN_URL, params);
		log.debug(token);
		if (token == null) {
			throw new RRException("获取token失败。");
		}
		if (token.contains("not in whitelist")) {
			throw new RRException("您的服务器不在微信授权IP白名单中。");
		}
		OAuth2Token o = parseStringAccessToken(token);
		return o;
	}
