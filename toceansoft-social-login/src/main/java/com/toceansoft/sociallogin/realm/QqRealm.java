...
public class QqRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
		...
		SysQqUserEntity sysQqUser = sysQqUserService.queryByOpenid(openid);
		if (sysQqUser == null) {
			throw new RRException("登陆过程保存QQ用户信息失败。");
		}
		SysUserQqUserEntity sysUserQqUser = sysUserQqUserService
				.queryByQqUserId(sysQqUser.getQqUserId());
		SysUserEntity user = null;
		if (sysUserQqUser == null) {
			// 尚未关联系统用户
			...
			if (config.isQqNeedBinding()) {
				throw new RRException("该QQ尚未绑定系统用户，请绑定后再尝试登陆。");
			}

			user = sysUserService.queryByUserName(sysQqUser.getNickname());
			if (user == null) {
				// 根据微信openid创建系统用户，兼容权限体系
				user = new SysUserEntity();
				user.setUsername(sysQqUser.getNickname());
			} else {
				// 已经存在与微信nickname同名的系统用户，修正之：一个唯一的随机字符串拼接上nickname，如果长度超过50，则截取50个字符
				user.setUserId(null);
				String username = ToceanUuid.getStringUuid() + sysQqUser.getNickname();
			...
			user.setUserAvatar(sysQqUser.getFigureurl());
			user.setUserType(UserTypeUtils.QQ_USER);
			...
			// 创建微信用户与系统用户关联关系
			sysUserQqUser = new SysUserQqUserEntity();
			sysUserQqUser.setUserId(user.getUserId());
			sysUserQqUser.setQqUserId(sysQqUser.getQqUserId());
			sysUserQqUserService.save(sysUserQqUser);

		} else {
			user = sysUserService.queryObject(sysUserQqUser.getUserId());
		}
		if (user == null) {
