...
public class WechatRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
		...
		SysWechatUserEntity sysWechatUser = sysWechatUserService.queryByOpenid(openid);
		if (sysWechatUser == null) {
			throw new RRException("登陆过程保存微信用户信息失败。");
		}
		SysUserWechatUserEntity sysUserWechatUser = sysUserWechatUserService
				.queryByWechatUserId(sysWechatUser.getWechatUserId());
		SysUserEntity user = null;
		if (sysUserWechatUser == null) {
			// 尚未关联系统用户
			...
			if (config.isWechatNeedBinding()) {
				throw new RRException("该微信尚未绑定系统用户，请绑定后再尝试登陆。");
			}

			user = sysUserService.queryByUserName(sysWechatUser.getNickname());
			if (user == null) {
				// 根据微信openid创建系统用户，兼容权限体系
				user = new SysUserEntity();
				user.setUsername(sysWechatUser.getNickname());
			} else {
				// 已经存在与微信nickname同名的系统用户，修正之：一个唯一的随机字符串拼接上nickname，如果长度超过50，则截取50个字符
				user.setUserId(null);
				String username = ToceanUuid.getStringUuid() + sysWechatUser.getNickname();
			...
			user.setUserAvatar(sysWechatUser.getHeadimgurl());
			user.setUserType(UserTypeUtils.WECHAT_USER);
			...
			// 创建微信用户与系统用户关联关系
			sysUserWechatUser = new SysUserWechatUserEntity();
			sysUserWechatUser.setUserId(user.getUserId());
			sysUserWechatUser.setWechatUserId(sysWechatUser.getWechatUserId());
			sysUserWechatUserService.save(sysUserWechatUser);

		} else {
			user = sysUserService.queryObject(sysUserWechatUser.getUserId());
		}
		if (user == null) {
